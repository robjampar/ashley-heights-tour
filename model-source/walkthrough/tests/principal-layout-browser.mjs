...
const out=new URL('../../revisions/interiors-principal-2026-09-26/layout/',import.meta.url);
...
try{
 for(const design of ['compact','planning']){
  ...
  await page.goto(root+'interiors/principal/model.html?design='+design);
  ...
  const result=await page.evaluate(()=>{
   const names=[];interiorPreview.scene.traverse(o=>{if(o.isMesh)names.push(...o.userData.spatialBatch?.sourceNames??[]);});
   return{revision:interiorPreview.info.layoutRevision,variant:interiorPreview.info.variant,
    authored:names.filter(n=>n.startsWith('Suite layout |')).length,
    features:['TV screen','bed mattress','sofa seat','wardrobe','desk top','dressing island'].map(term=>({term,count:names.filter(n=>n.includes(term)).length})),
    views:Object.keys(interiorPreview.views),batches:interiorPreview.batches};
  });
  assert.equal(result.revision,1);assert.equal(result.variant,design);assert(result.authored>300);assert(result.features.every(f=>f.count>0));
  for(const view of result.views){
   await page.locator(`[data-camera=${view}]`).click();await page.waitForTimeout(450);
 ...
 await page.goto(root+'interiors/principal/model.html?design=planning&view=bed');
 await page.waitForFunction(()=>window.interiorPreview?.ready,null,{timeout:120000});
 ...
 await page.setViewportSize({width:1200,height:1080});await page.goto(planURL);assert.equal(await page.locator('svg title').textContent(),'Principal suite — revised layout');
 await page.screenshot({path:new URL('plan.png',out).pathname});
 assert.deepEqual(errors,[]);assert.deepEqual(failed,[]);
 await fs.writeFile(new URL('browser-checks.json',out),JSON.stringify({status:'PASS',designs:checks,mobileWidth:390,retainedCameraAcrossDesigns:true,errors,failed},null,2)+'\n');
 console.log('PASS: both native suite studies, all seven cameras, retained camera on reload/design change, measured SVG, mobile layout, no browser errors.');
