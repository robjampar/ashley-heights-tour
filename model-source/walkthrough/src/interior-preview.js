...
const lighting=daylight(renderer,scene,camera),controls=new OrbitControls(camera,canvas);controls.enableDamping=true;controls.minDistance=.16;controls.maxDistance=15;controls.maxPolarAngle=Math.PI*.99;
...
try{
 ...
 fills=roomLights(scene,[{name:'Kitchen daylight bounce',position:[2.15,6.40,1.55],range:4.2,intensity:1.65},...(info.proposalLights??[])],{budget:6});lighting.updateShadows();ready=true;$('model-status').hidden=true;
 await vegetation.buildSite(STREET_DATA.siteTrees.filter(t=>t.kind!=='row'||(t.from?.[1]>8&&t.to?.[1]>8)));lighting.updateShadows();
