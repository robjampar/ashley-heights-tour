...
OUT=ROOT/'photo-review/porch-width-proof';OUT.mkdir(exist_ok=True)
...
width_review=refine_porch_width(globals())
...
snap=snapshot();r=refine_front_arch_windows(globals());width_review=refine_porch_width(globals());snap2=snapshot();idem=snap==snap2
...
for ob in bpy.data.objects:
 if ob.type!='MESH' or not(ob.name.startswith('Front arch correction | ')or ob.name in r['cut_meshes'] or ob.name in width_review['objects']):continue
...
(OUT/'geometry-validation.json').write_text(json.dumps({'helper':r,'width_review':width_review,'idempotent':idem,'dimension_checks':checks,'mesh_checks':mesh_checks,'mesh_failures':failed},indent=2))
assert idem and not failed
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'Porch width proof.blend'))
