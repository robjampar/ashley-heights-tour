...
Complete current-design drawing packs now use the same verified-reuse approach, including rendering-library versions and output checksums. Each unchanged pack took about **1.0 s** instead of about **169 s** to redraw. The standard command refreshing both current models, both drawing packs and the nine-design viewer took **5.33 s** with all inputs unchanged. `--force` redraws; partial sheet previews do not replace or reuse a complete issue.
