...
Open `interiors/principal/` for ten bedroom furnishing concepts continuing the selected oak, ivory and warm stone palette. This is a preference board, not a fitted native room yet. The two current proposals share the principal suite layout, so the ten images apply to either; notes and marks stay separate by design and by room. Current bedroom, sitting-area and study captures provide context. `walkthrough/tools/capture-principal-room.mjs` records their exact cameras from the built model.
