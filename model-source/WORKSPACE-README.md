...
The five floorplans below were not selected; the owner particularly disliked the bed/sofa/TV arrangement. Open `interiors/principal/arrangements.html` for three focused furniture studies: parallel furniture with one fixed screen, a pivoting screen used by one area at a time, and two direct-view screens. The adjoining rooms remain unresolved. Run `.venv/bin/python scripts/draw_principal_arrangements.py` and `node walkthrough/tests/principal-arrangements-browser.mjs`. Checks cover door sweep, occupied-chair footprints, 800 mm routes and sampled viewing geometry; they do not establish full-suite, native headroom or construction feasibility. Research is in `revisions/interiors-principal-2026-09-26/LAYOUT-RESEARCH.md`.
