...
The model tour offers three designs: the original house (`model/`), **Proposal A** (`model/?design=proposed`) and the smaller **Proposal B (compact)** (`model/?design=compact`), whose new wing is about 25% shorter along the front, taken off the garage end on all three floors with the same depth, its ridge lowered to the existing ridge and its loft a long east dormer over a full-width loft floor. The button under the header compares the current proposal with the original, keeping the viewpoint; a second button swaps between the two proposals.
